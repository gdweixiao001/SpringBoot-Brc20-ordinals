...
public class BaseController {
     ...
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
    protected TableDataInfo getDataTable(IPage<Map<String, Object>> pages) {
        //处理大数据
        ...
        if (Func.isNotEmpty(records)) {
            records.forEach(dataMap -> {
                Map<String, Object> tmpMap = new HashMap<>();
                dataMap.forEach((key, value) -> {
                    if (value instanceof Long) {
                        tmpMap.put(key, Func.toStr(value));
                    } else {
                        tmpMap.put(key, value);
                    }
                });
                tmpList.add(tmpMap); // 在处理完dataMap的所有键值对后添加tmpMap到tmpList
            });
        }

        TableDataInfo rspData = new TableDataInfo();
        rspData.setCode(0);
        rspData.setRows(tmpList);
