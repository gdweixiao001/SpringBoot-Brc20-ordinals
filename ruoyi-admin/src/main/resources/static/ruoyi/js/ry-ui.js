...
(function ($) {
    $.extend({
        ...
        table: {
            // 初始化表格参数
            init: function(options,tableId,toolbarId) {
            ...
            // 图片预览
            imageView: function (value, height, width, target,classstr) {
                ...
                if ($.common.isNotEmpty(value)) {
                    return $.common.sprintf("<img class='img-circle img-xs "+classstr+"' data-height='%s' data-width='%s' data-target='%s' src='%s'/>", height, width, _target, value);
                } else {
                    return $.common.nullToStr(value);
                }
            },
            // 判断是否为空
            isnull: function (title, value, other) {
        ...
        operate: {
            ...
            add: function(id) {
                ...
                $.modal.open(title, $.operate.addUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
            ...
            // 打开 全屏
            openFull: function(id,openUrl,title) {
                if(title==null || title==''){
                    title ='详情';
                }
                table.set();
                var url = "/404.html";
                url = openUrl.replace("{id}", id);
                $.modal.openFull(title, url);
            },
            openTablFull: function(id,openUrl,title) {
                if(title==null || title==''){
                    title ='详情';
                }
                table.set();
